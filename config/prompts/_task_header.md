...
If you claim a task, investigate, and find the work is already done (tests pass, code already exists), just signal `COMPLETE_AND_READY_FOR_MERGE(reason)`. The framework will complete claimed tasks even without a code diff. Include why the task is satisfied so there's provenance.
...
- **`COMPLETE_AND_READY_FOR_MERGE`** or **`COMPLETE_AND_READY_FOR_MERGE(notes)`** — Your work is done and ready for review. Optional notes are saved to task metadata and commit message.
